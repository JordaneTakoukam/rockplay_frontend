...
import { userGoogleLogin, 
    // metamaskLogin, getMyBalances,
     emailLogin, verifyEmailCode, updateProfileSet } from "redux/actions/auth";
...
const useStyles = makeStyles(() => ({
    ModalBox: {
        ...
        background: '#2C2C3A',
        position: 'fixed', // Assurez-vous que la position est fixed pour centrer par rapport à l'écran
        display: 'flex',
        alignItems: 'center',
        ...
        padding: '30px 0', // Padding en haut et en bas
...
const AuthenticationModal = ({ open, setOpen, authType }) => {
    ...
    const handleEmailLogin = async () => {
        if (vaildateEmailAddress(emailAddress)) {
            ...
            if (response.status) {
                setCodeInput(true);
                addToast('Code sent successfully!', { appearance: 'success', autoDismiss: true });
    ...
    const sendVerifyCode = async (value) => {
        ...
        if (response.status) {
            ...
            dispatch({ type: 'INIT_SETTING', data: settingData });
            addToast('Welcome! We are glad to have you.', { appearance: 'success', autoDismiss: true });

        }
        else {
            addToast('The code is incorrect or has expired.', { appearance: 'error', autoDismiss: true });
        }
        hideLoading();
    }
    ...
    const handleResendCode = async () => {
        ...
        const response = await emailLogin(request);
        if (response.status) {
            addToast('Code resent successfully!', { appearance: 'success', autoDismiss: true });
        }
        else {
            addToast(response.message, { appearance: 'error', autoDismiss: true });
        }
