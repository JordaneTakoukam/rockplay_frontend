...
export class CrashLayer extends BaseLayer {
    ...
    createCountdown() {
        ...
        const style = new PIXI.TextStyle({
            ...
            fill: '0xFFFFFF',
            lineHeight: 10, // Réduire la hauteur de ligne si nécessaire
            align: 'center',
        });

        // Ajouter le texte avec la taille ajustée
        ...
        // Si tu as un bouton, tu peux également ajuster sa taille (réduire la hauteur)
        if (this.button) {
            this.button.scale.set(1, 0.5); // Réduire uniquement la hauteur du bouton à 50% de sa taille originale
        }
    ...
    createExplosion(resource) {
        ...
        this.spr_explosion.scale.set(gameApp.scale_x * 1.2); // Réduction du facteur d'échelle
    ...
    renderBg() {
        ...
        // Multiplier la taille en fonction de l'appareil
        const scaleFactor = isMobile ? 2.5 : 1.5;

        // Appliquer les transformations
        this.spr_bg.setTransform(
            gameApp.canvas_width / 2,
            gameApp.canvas_height / 2
        );
        this.spr_bg.anchor.set(0.5);
        this.spr_bg.scale.set(gameApp.max_scale * scaleFactor);
    }
    ...
    renderRocket(frame, state) {
        ...
        // Modifié la couleur à jaune (0xFFFF00) et réduit la largeur de la ligne à 10 pixels
        this.graphic_curve.lineStyle(6.0 * gameApp.scale_x, 0xF9D7D4, 1);  // Ligne jaune avec une largeur réduite
        ...
        this.spr_rocket.scale.set(gameApp.scale_x * 0.75 * 0.23);  // Réduit la taille de la fusée à 30% de la taille originale
